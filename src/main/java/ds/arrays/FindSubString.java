...
public class FindSubString {
	...
	public static void main(String[] args) {
		...
		System.out.println(foundIndex > -1 ? inputString + " contains "
				+ searchSubString + " at index " + foundIndex : inputString
				+ "doesn't contains " + searchSubString);
		javaApiFoundIndex = inputString.indexOf("tiger");
		System.out.print("Java API's indexOf() method says> ");
		System.out.println(javaApiFoundIndex > -1 ? inputString + " contains "
				+ searchSubString + " at index " + javaApiFoundIndex
	...
	public static int isSubString(String inputString, String searchSubString) {
		...
		for (int i = 0; i < inputStringAr.length; i++) {
			...
			if (String.valueOf(inputStringAr[i]).equals(
					String.valueOf(searchSubStringAr[count]))) {
				...
				/*
				 * Below if condition comes into role, when some part of
				 * searchSubString is present before whole searchSubString. Eg=
				 * searchSubString=tiger & inputString="tigtiger" [here, (tig)
				 * is present before (tiger)]
				 */
				if (count > 0) {
					count = 0;
					i--;
